...
class Autoescuela(models.Model):
    ...
    profesor_ids = fields.One2many(
        'autoescuela.profesor', 'autoescuela_id', string="Profesores"
    )
    alumno_ids = fields.One2many(
        'autoescuela.alumno', 'autoescuela_id', string="Alumnos"
    )
    examen_ids = fields.Many2many(
        'autoescuela.examen', string="Exámenes"
    )

    # Contadores
    alumnos_count = fields.Integer(
        string="Número de alumnos", compute='_compute_counts'
    )
    profesores_count = fields.Integer(
        string="Número de profesores", compute='_compute_counts'
    )
...
class Profesor(models.Model):
    ...
    autoescuela_id = fields.Many2one(
        'autoescuela.autoescuela', string="Autoescuela"
    )
    incorporacion = fields.Date(string="Fecha de incorporación")
    antiguedad = fields.Integer(
        string="Antigüedad (años)", compute='_compute_antiguedad'
    )

    alumno_ids = fields.One2many(
        'autoescuela.alumno', 'profesor_id', string="Alumnos"
    )
    ...
    def _compute_antiguedad(self):
        for rec in self:
            if rec.incorporacion:
                hoy = date.today()
                rec.antiguedad = (
                    hoy.year - rec.incorporacion.year
                    - ((hoy.month, hoy.day) <
                       (rec.incorporacion.month, rec.incorporacion.day))
                )
            else:
                rec.antiguedad = 0
...
class Alumno(models.Model):
    ...
    profesor_id = fields.Many2one(
        'autoescuela.profesor', string="Profesor asignado"
    )
    examen_ids = fields.One2many(
        'autoescuela.examen', 'alumno_id', string="Exámenes"
    )

    domicilio = fields.Char(string="Domicilio")
    matricula = fields.Integer(string="Matrícula")
    fecha_nacimiento = fields.Date(string="Fecha de nacimiento")
    numero_practicas = fields.Integer(
        string="Prácticas realizadas", default=0
    )
...
class Examen(models.Model):
    ...
    autoescuela_ids = fields.Many2many(
        'autoescuela.autoescuela', string="Autoescuelas"
    )

    moneda_id = fields.Many2one('res.currency', string="Moneda")
    precio = fields.Monetary(
        string="Precio", currency_field='moneda_id'
    )
        ...
        for rec in self:
            if rec.clases is not None and rec.clases < 0:
                raise ValidationError(
                    "El número de clases no puede ser negativo."
                )
